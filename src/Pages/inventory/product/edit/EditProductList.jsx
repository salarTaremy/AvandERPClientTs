...
const EditProductList = () => {
  ...
  const [brandData, brandLoading, brandError, brandApiCall] =
    useFetchWithHandler();
  const [seasonalReportsdata, seasonalReportsloading, seasonalReportserror] =
  ...
  useEffect(() => {
    selectedSupplier &&
      brandApiCall(
        `${url.BRAND_GET_WITH_PERMISSION}?supplierId=${selectedSupplier}`,
      );
  }, [selectedSupplier]);
  ...
  useEffect(() => {
    selectedUnitType &&
      unitApiCall(`${url.PRODUCT_UNIT}?productUnitTypeId=${selectedUnitType}`);
  }, [selectedUnitType]);
            ...
            <Ant.Card title={"اطلاعات پایه"} type="inner">
              <Ant.Row gutter={[16, 8]}>
                <Ant.Col span={24} sm={8}>
                  ...
                  >
                    <Ant.Input
                      placeholder="نام کالا/خدمت' به صورت فارسی"
                      allowClear
                    />
                  </Ant.Form.Item>
                </Ant.Col>
