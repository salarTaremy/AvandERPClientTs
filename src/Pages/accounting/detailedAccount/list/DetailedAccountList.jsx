...
const DetailedAccountList = () => {
  ...
  const onAdd = () => {
    setModalContent(<FrmAddDetailedAccount onSuccess={onSuccessAdd} key={uuid.v1()} />)
    setModalState(true)
  }
  ...
  const onEdit = (val) => {
    setModalContent(<FrmEditDetailedAccount onSuccess={onSuccessEdit}  id={val.id} key={val.id} obj={val} />)
    setModalState(true)
  }
  const onView = (id) => {
